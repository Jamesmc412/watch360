(() => {
  ...
  const getCurrentVideoInfo = () => {
    const videoTitle = document.querySelector('h1.title.style-scope.ytd-video-primary-info-renderer')?.innerText || "Unknown title";
    const channelName = document.querySelector('#owner-name a')?.innerText || "Unknown channel"; // Updated selector
    const videoLength = youtubePlayer?.duration || 0;
  ...
  setInterval(() => {
    youtubePlayer = document.querySelector('.video-stream'); // Improved query selector for the video element
    if (youtubePlayer) {
